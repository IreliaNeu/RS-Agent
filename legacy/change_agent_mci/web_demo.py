...
from lagent.actions import ActionExecutor, GoogleSearch, PythonInterpreter, Visual_Change_Process_PythonInterpreter, ConvertToMultimodalDataset, RefineCaptionsWithLLM
...
class SessionState:
    ...
    def init_state(self):
        ...
        action_list = [Visual_Change_Process_PythonInterpreter(), ConvertToMultimodalDataset(), GoogleSearch(),RefineCaptionsWithLLM()]
...
class StreamlitUI:
    ...
    def setup_sidebar(self):
        ...
        uploaded_file_dir_A = st.sidebar.file_uploader(
            '**Upload Image Directory_A(.zip):**', type=['zip'])
        uploaded_file_dir_B = st.sidebar.file_uploader(
            '**Upload Image Directory_B(.zip):**', type=['zip'])
        return model_name, model, plugin_action, uploaded_file_A, uploaded_file_B, uploaded_file_dir_A, uploaded_file_dir_B 
        ...
        if option not in st.session_state['model_map']:
            if option.startswith('gpt'):
                ...
                st.session_state['model_map'][option] = HFTransformerCasualLM(
                    ...
                    # load_in_4bit=True  # 4-bit量化（需8GB显存）
                )
        # print(f"Current device map: {model.model.device_map}")  # 应显示全部在cuda:0
        # print(f"First layer weight device: {next(model.model.parameters()).device}")  # 应为cuda:0        
...
def main():
    ...
    if 'ui' not in st.session_state:
        ...
        st.set_page_config(
            ...
            page_icon='./docs/imgs/lagent_icon.png')
        st.header('🌏:blue[RS] Agent ', divider='rainbow')
    model_name, model, plugin_action, uploaded_file_A, uploaded_file_B, uploaded_file_dir_A, uploaded_file_dir_B = st.session_state[
        'ui'].setup_sidebar()
    ...
    if user_input := st.chat_input(''):
        ...
        if uploaded_file_A:
            ...
            file_name_A = f'A_{uploaded_file_A.name}'
            file_path_A = os.path.join(root_dir, file_name_A)
            with open(file_path_A, 'wb') as tmpfile:
                tmpfile.write(file_bytes_A)
            st.write(f'File A saved at: {file_path_A}')
        ...
        if uploaded_file_B:
            ...
            file_name_B = f'B_{uploaded_file_B.name}'
            file_path_B = os.path.join(root_dir, file_name_B)
            with open(file_path_B, 'wb') as tmpfile:
                tmpfile.write(file_bytes_B)
            st.write(f'File B saved at: {file_path_B}')

        # 拼接 user_input 时用已保存的路径
        if uploaded_file_A and uploaded_file_B:
            user_input = (
                f'The path of the image_A: {file_path_A}. '
                f'The path of the image_B: {file_path_B}. '
                f'{user_input}'
            )
        
        folder_A_path, folder_B_path = None, None
        if uploaded_file_dir_A:
            dir_A_name = f"A_dir_{uploaded_file_dir_A.name.replace('.zip','')}"
            folder_A_path = os.path.join(root_dir, dir_A_name)
            os.makedirs(folder_A_path, exist_ok=True)
            with zipfile.ZipFile(uploaded_file_dir_A, 'r') as zip_ref:
                zip_ref.extractall(folder_A_path)
            st.write(f" 已解压前时相图像文件夹到: `{folder_A_path}`")

        # 新增：解压后时相图像文件夹
        if uploaded_file_dir_B:
            dir_B_name = f"B_dir_{uploaded_file_dir_B.name.replace('.zip','')}"
            folder_B_path = os.path.join(root_dir, dir_B_name)
            os.makedirs(folder_B_path, exist_ok=True)
            with zipfile.ZipFile(uploaded_file_dir_B, 'r') as zip_ref:
                zip_ref.extractall(folder_B_path)
            st.write(f" 已解压后时相图像文件夹到: `{folder_B_path}`")
        
        if uploaded_file_dir_A and uploaded_file_dir_B:
            user_input = (
                f"前时相图像文件夹 `{folder_A_path}` 后时相图像文件夹 `{folder_B_path}` "
                f'{user_input}'
            )
