...
sys.path.append('D:/Deeplearning/Change_Agent_main/Multi_change')
import os.path
...
class Change_Perception(object):
    def define_args(self):
        ...
        parser.add_argument('--data_folder', default=r'D:\Deeplearning\Change_Agent_main\Multi_change\Levir-MCI-dataset\images',
                            help='folder with data files')
        parser.add_argument('--list_path', default=r'D:\Deeplearning\Change_Agent_main\Multi_change\data\LEVIR_MCI/',
    ...
    "生成多样化文本描述（测试）"
    def generate_change_caption_multi(self, path_A, path_B):
        print('model_infer_change_captioning_multi: start')
        imgA, imgB = self.preprocess(path_A, path_B)
        # Move to GPU, if available
        imgA = imgA.cuda()
        imgB = imgB.cuda()
        feat1, feat2 = self.encoder(imgA, imgB)
        feat1, feat2, seg_pre = self.encoder_trans(feat1, feat2)
        seq = self.decoder.sample_beam(feat1, feat2, k=3)
        pred_seq = [w for w in seq if w not in {self.word_vocab['<START>'], self.word_vocab['<END>'], self.word_vocab['<NULL>']}]
        pred_caption = ""
        for i in pred_seq:
            pred_caption += (list(self.word_vocab.keys())[i]) + " "
    ...
    "单模态变化检测数据集转化为多模态变化检测数据集"
    def convert_to_multimodal(self, input_dir_A, input_dir_B, output_dir):

        """
        批量将图像对转换为多模态数据（图像 + 文本描述）
        Args:
            input_dir_A (str): 前时相图像所在路径
            input_dir_B (str): 后时相图像所在路径
            output_dir (str): 输出文本描述保存目录
        Returns:
            str: JSON 格式的摘要信息（成功/失败数、错误文件）
        """
        os.makedirs(output_dir, exist_ok=True)
        filenames = sorted(os.listdir(input_dir_A))
        count = 0
        errors = []

        for filename in filenames:
            path_A = os.path.join(input_dir_A, filename)
            path_B = os.path.join(input_dir_B, filename)
            ...
            try:
                caption = self.generate_change_caption(path_A, path_B)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Remote_Sensing_Image_Change_Interpretation')
    parser.add_argument('--imgA_path', default=r'D:/Deeplearning/Change_Agent_main/Multi_change/predict_result/test_000004_A.png')
    parser.add_argument('--imgB_path', default=r'D:/Deeplearning/Change_Agent_main/Multi_change/predict_result/test_000004_B.png')
