...
def generate_caption_from_image(image_path, llm, args):
    ...
    prompt = (
       "<think> You are a professional remote sensing imagery analysis assistant." 
        "Your task is to interpret and describe land surface features in satellite/aerial imagery with scientific accuracy." 
        "You should first analyze the image to identify key elements (e.g., land cover types, city and road, spectral signatures)." 
        "Then, using your background knowledge in remote sensing (e.g., NDVI for vegetation, texture for urban areas), generate a precise description of the observed features." 
        "When appropriate, incorporate domain knowledge (e.g., phenological cycles, disaster impacts) to enhance interpretation." 
        "Respond in JSON format with two fields: 'reasoning' and 'image description'.</think>" 
        )
...
def judge_best_option(options, args, image_path, max_retries=5, retry_delay=2.0):
...
def score_options(options, args, image_path, max_retries=5, retry_delay=2.0):
