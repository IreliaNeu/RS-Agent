...
def call_openrouter(llm, messages, temperature, max_tokens, max_retries=5, retry_delay=2.0):
    """
...
def refine_caption(original, llm, args):
    ...
    system_prompt = (
        "<think> You are a remote sensing imagery analysis assistant."
        "Given an input describing potential land surface features in satellite/aerial imagery," 
        "first analyze how the description could be improved in terms of clarity, completeness, or scientific accuracy." 
        "Then, refine the description without changing its core observations. "
        "Return your reasoning as a short paragraph, followed by the final improved description. </think> "
    )
    user_prompt = f"""
    ...
    return call_openrouter(
        ...
        max_tokens=args.judge_max_tokens * 40  # allow enough space
    )
